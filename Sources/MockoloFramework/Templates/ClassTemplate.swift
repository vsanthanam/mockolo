...
extension ClassModel {
    func applyClassTemplate(name: String,
        ...
        let extraInits = extraInitsIfNeeded(initParamCandidates: initParamCandidates, declaredInits: declaredInits,  acl: acl, declType: declType, overrides: metadata?.varTypes, typeKeys: typeKeys)
    ...
    private func extraInitsIfNeeded(initParamCandidates: [Model],
        ...
        if needBlankInit {
            ...
            blankInit = "\(acl)init() { \(String.doneInit) = true }"
        }
